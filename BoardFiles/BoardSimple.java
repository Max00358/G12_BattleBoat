...
public class BoardSimple {
	...
	public static void main(String[] args) throws Exception {
		...
		while(restart == 1){//Create a while loop to loop the battle boat game
			...
			Background letsGo = new Background("daBaby.wav");
			...
			//Create a while loop that contains a nested for loop to make sure the random coordinates don't repeat themselves
			while(repeat == true){
				for(int c = 0; c < randomRow.length; c++){
					for(int r = c+1; r < randomRow.length; r++){
						
						//If two elements in randomRow equal to each other and two elements in randomCol equals to each other, then add 1 to the element on index c in an array named randomCol
						if(randomRow[c] == randomRow[r] && randomCol[c] == randomCol[r]){
							//If the element on index c of array randomCol has a value of 9, then set the value of that element to 0
							if(randomCol[c] == randomRow.length - 1){
								randomCol[c] = 0;
							}
							//Else increase the value of the element on index c of array randomCol by 1
							else{
								randomCol[c] = randomCol[c] + 1;
							}
						}
						//End the while loop otherwise because there are no repeating coordinate sets
						else{
							repeat = false;
						}
					}
			...
			while (unWon) {
				...
				for (int c = 0; c < randomRow.length; c++) {
					...
					//Else if the check method returns a checkRandom value that's true and if the coordinate that the user clicked on on the boolean 2D array states "false", then put a "sheesh" emoji because the user guessed the random coordinate
						...
						//When the user guess the random coordinate and the total target that the user hit is less than 10, then play the background music named "sheesh" 
						if(sheeshNum < 10){
							sheesh.run();
						}
						...
						board.displayMessage("Still need " + (10-sheeshNum) + " hits to win");//Inform user how many more targets they need to hit
						
						//If user hits all ten targets, play the background music named "letsGo" and end the for loop as well as the while loop
						if (sheeshNum == 10) {
							letsGo.run();
							unWon = false;
							c = randomRow.length;
